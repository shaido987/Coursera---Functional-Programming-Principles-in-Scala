...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
      ...
      val accBoth  = left.filterAcc(p, accRight)
      if (p(elem)) accBoth.incl(elem)
      else accBoth 
    }
    ...
    def mostRetweetedAcc(curr: Tweet): Tweet = {
      right.mostRetweetedAcc(left.mostRetweetedAcc(if (curr.retweets > elem.retweets) curr else elem))
    }
